...
class CommentCreateView(views.APIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            comment = self.service.create_comment(**serializer.validated_data, request.user)
            serializer = self.serializer_class(comment)
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        # comment = self.service.create_comment(request.data, request.user)

    def get(self, request):
        comments = self.service.get_comments(request.user)
...
class CommentDetailsView(views.APIView):
    ...
    def patch(self, request, id):
        comment = self.service.update_comment(id, request.data)
        serializer = self.serializer_class(comment)
        return Response(serializer.data)